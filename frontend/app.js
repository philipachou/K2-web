...
import { buildImageCard, handleImageLoadError, formatMarkdownContent, msgToHtml, createChatMessageElement, renderSingleChatMessage as chatRenderSingleChatMessage, renderChatLog as chatRenderChatLog } from './js/chat.js';
import { DEFAULT_SETTINGS, syncSettingsModalUI as settingsSyncModalUI, updateSettingsVisibility as settingsUpdateVisibility, parseCSVGeneric, parseContactsCSV, parseActionsCSV as settingsParseActionsCSV, exportConfiguration as settingsExportConfig, importConfiguration as settingsImportConfig } from './js/settings.js';
...
function syncSettingsModalUI() {
  settingsSyncModalUI(settings, updateSettingsVisibility);
}

function updateSettingsVisibility() {
  settingsUpdateVisibility();
}
...
async function exportConfiguration() {
  await settingsExportConfig(db);
}

async function importConfiguration(file) {
  await settingsImportConfig(db, file);
}
...
function parseActionsCSV(text) {
  return settingsParseActionsCSV(text, getCurrentSeparator());
}
...
function renderSingleChatMessage(msg) {
  chatRenderSingleChatMessage(msg, applySuggestionToEditor);
}
window.renderSingleChatMessage = renderSingleChatMessage;

async function renderChatLog(force = false) {
  await chatRenderChatLog(getChatHistory, force, applySuggestionToEditor);
}
